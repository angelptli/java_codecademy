...
------------------------------------
One common pattern we'll encounter as a programmer is traversing, or looping,
through a list of data and doing something with each item. In Java, that list
would be an array or ArrayList and the loop could be a for loop. But wait, how
does this work?
...
with a value of 0 and increment until the end of the list. So we can increment
through the array or ArrayList using its indices.

For example, if we wanted to add 1 to every int item in an array secretCode,
we could do this:
//////////////////////////////////////////////////
...
array indices start at 0, the length of secretCode is 1 larger than its final
index. A loop should stop its traversal before its counter variable is equal
...
ArrayIndexOutOfBoundsException error! This is a very common mistake when first
starting to traverse arrays.
...
use a whlie loop, we need to create our own counter variable to access
...
Instructions
------------
1. Let's use a for loop to iterate over expenses and sum up the total of all
   items. Start with the skeleton of a for loop:
