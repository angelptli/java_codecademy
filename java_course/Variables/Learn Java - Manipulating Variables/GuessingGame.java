...
that can hold big decimal numbers. But how would we deposit and withdraw from
the account?
...
In the filnal line of the code above, we used the expression balance +
depositAmount to determin the new value of the balance variable. When an
expression is executed, it produces a single value.

The data type of a variable plays a large role in the operations we can use
to manipulate it. We can think of a data type as a combination of a set of
values, and a set of operations on those values. For example, the double data
type is comprised of values like 4.8 and operations like addition (+). For now,
we'll mainly focus on the set of operations that can be used on numbers and
booleans.
...
Instructions
------------
1. In the file GuessingGame.java, we have defined two integers mystery1 and
   mystery2. We will talk about these operators, among others, in the rest of
   the lesson. Use System.out.println() to print the variable that holds a
   value of 2.
*/
